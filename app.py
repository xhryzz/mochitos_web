...
def api_media_search():
    ...
    # Tokenizamos (espacios) y soportamos exclusiones con prefijo '-'
    raw_tokens = [t.strip() for t in re.split(r'\s+', q) if t.strip()]
    pos_tokens, neg_tokens = [], []
    for t in raw_tokens:
        (neg_tokens if t.startswith('-') else pos_tokens).append(t.lstrip('-').lower())

    # Mapeo de sinónimos / typos comunes
    def norm_platform(tok: str) -> str | None:
        netflix_typos = {'netflix','netlfix','netflx','netfliz','netfli','netlix'}
        prime_typos   = {'prime','amazon','amzn','prmie','amzprime'}
        if tok in netflix_typos: return 'netflix'
        if tok in prime_typos:   return 'prime'
        return None

    # Filtros derivados de tokens
    want_netflix = any(norm_platform(t) == 'netflix' for t in pos_tokens)
    want_prime   = any(norm_platform(t) == 'prime'   for t in pos_tokens)
    not_netflix  = any(norm_platform(t) == 'netflix' for t in neg_tokens)
    not_prime    = any(norm_platform(t) == 'prime'   for t in neg_tokens)

    # Estado visto / por ver
    want_watched_true  = any(t in {'visto','vista','vistas','watched'} for t in pos_tokens)
    want_watched_false = any(t in {'por','ver','porver','pendiente','pendientes','unwatched','novista'} for t in pos_tokens)
    # Prioridad
    prio_map = {'alta','media','baja'}
    want_prios = {t for t in pos_tokens if t in prio_map}

    # Términos genéricos (texto) → título o comentario
    def is_generic(tok: str) -> bool:
        if tok in prio_map: return False
        if norm_platform(tok) is not None: return False
        if tok in {'visto','vista','vistas','watched','por','ver','porver','pendiente','pendientes','unwatched','novista'}:
            return False
        return True
    text_pos = [t for t in pos_tokens if is_generic(t)]
    text_neg = [t for t in neg_tokens if is_generic(t)]

    # Construimos SQL dinámico pero seguro (parametrizado)
    where = ["1=1"]
    params = []

    if want_netflix: where.append("on_netflix = TRUE")
    if want_prime:   where.append("on_prime   = TRUE")
    if not_netflix:  where.append("on_netflix = FALSE")
    if not_prime:    where.append("on_prime   = FALSE")

    if want_watched_true and not want_watched_false:
        where.append("is_watched = TRUE")
    elif want_watched_false and not want_watched_true:
        where.append("is_watched = FALSE")
    # si no se especifica, devuelve ambos

    if want_prios:
        where.append("COALESCE(priority,'media') = ANY(%s)")
        params.append(list(want_prios))

    # búsqueda por texto (case-insensitive) sobre título y comentario
    for t in text_pos:
        where.append("(title ILIKE %s OR comment ILIKE %s)")
        like = f"%{t}%"
        params.extend([like, like])

    for t in text_neg:
        where.append("NOT (title ILIKE %s OR comment ILIKE %s)")
        like = f"%{t}%"
        params.extend([like, like])
    ...
    sql = f"""
        ...
        ORDER BY
          is_watched ASC,
          CASE COALESCE(priority,'media')
              WHEN 'alta' THEN 0 WHEN 'media' THEN 1 ELSE 2
          END,
          COALESCE(created_at, '1970-01-01') DESC,
          id DESC
        LIMIT 200
    ...
    # Para que “se sienta” como el buscador de viajes, devolvemos ambas listas
    to_watch = [r for r in rows if not r['is_watched']]
    watched  = [r for r in rows if r['is_watched']]
