...
def init_db():
    with closing(get_db_connection()) as conn, conn.cursor() as c:
        ...
        # === MOCHIREAL ===
        c.execute("""
        CREATE TABLE IF NOT EXISTS mochireal_windows (
            id SERIAL PRIMARY KEY,
            date TEXT UNIQUE,       -- 'YYYY-MM-DD' (solo uno por día)
            triggered_at TEXT       -- Momento exacto en que se mandó la notificación
        )
        """)
        
        c.execute("""
        CREATE TABLE IF NOT EXISTS mochireal_posts (
            id SERIAL PRIMARY KEY,
            window_id INTEGER REFERENCES mochireal_windows(id),
            username TEXT NOT NULL,
            image_data BYTEA,
            mime_type TEXT,
            uploaded_at TEXT,
            UNIQUE(window_id, username)
        )
        """)


        c.execute("""
        CREATE TABLE IF NOT EXISTS mochireal_favorites (
            user_from TEXT NOT NULL,
            post_id   INTEGER NOT NULL REFERENCES mochireal_posts(id) ON DELETE CASCADE,
            created_at TEXT,
            PRIMARY KEY (user_from, post_id)
        )
        """)
...
def push_mochireal_upload(uploader: str):
    other = other_of(uploader)
    send_push_to(other,
        title="📸 MochiReal subido",
        body=f"{uploader} ha subido su foto. ¡Sube la tuya para desbloquearla!",
        url="/#mochireal",
        tag="mochireal-upload"
    )
...
# 8) Wishlist add (acepta form o JSON y nombres antiguos/nuevos)
            ...
            # --- MOCHIREAL DATA ---
            # Obtenemos la ventana más reciente
            c.execute("SELECT id, triggered_at FROM mochireal_windows ORDER BY id DESC LIMIT 1")
            mr_window = c.fetchone()
            
            mr_data = {
                "active": False,
                "my_post": None,
                "other_post": None,
                "is_locked": True # Asumimos bloqueado hasta comprobar
            }
            
            if mr_window:
                mr_data["active"] = True
                mr_data["triggered_at"] = mr_window[1]
                win_id = mr_window[0]
                
                # Buscamos los posts de esta ventana
                c.execute("SELECT id, username, uploaded_at FROM mochireal_posts WHERE window_id=%s", (win_id,))
                posts = c.fetchall()
                
                for p in posts:
                    pid, p_user, p_time = p[0], p[1], p[2]
                    if p_user == user:
                        mr_data["my_post"] = {"id": pid, "time": p_time}
                    else:
                        mr_data["other_post"] = {"id": pid, "time": p_time}
                
                # Regla de Oro: Si yo he subido foto, se desbloquea la del otro
                if mr_data["my_post"]:
                    mr_data["is_locked"] = False
...
@app.post('/mochireal/upload')
def mochireal_upload():
    if 'username' not in session: return redirect('/')
    user = session['username']
    
    if 'photo' not in request.files:
        flash("No has seleccionado ninguna foto", "error")
        return redirect('/#mochireal')
        
    file = request.files['photo']
    if not file.filename: return redirect('/#mochireal')

    # Buscar la ventana activa (la última creada)
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT id, triggered_at FROM mochireal_windows ORDER BY id DESC LIMIT 1")
