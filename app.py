...
# ========== [TRIVIA] Constantes ==========
TRIVIA_CATEGORIES = ('Arte','Ciencia','Geografía','Historia','Deporte','Entretenimiento')
TRIVIA_DAILY_RESET_HHMM = "10:00"  # Europe/Madrid

TRIVIA_TIME_PER_QUESTION_SEC = 15
TRIVIA_SYNC_WINDOW_HOURS = 6

def _iso_week_str(d: date) -> str:
    y, w, _ = d.isocalendar()
    return f"{y}-W{w:02d}"

def _today_week_str() -> str:
    return _iso_week_str(today_madrid())
...
def _rand_from_seed(seed: str, a: int, b: int) -> int:
    # entero determinista a..b usando sha256
    h = hashlib.sha256(seed.encode('utf-8')).hexdigest()
    x = int(h[:12], 16)
    return a + (x % (b - a + 1))

def _shuffle_with_seed(items: list, seed: str) -> list:
    arr = list(items)
...
def init_db():
    with closing(get_db_connection()) as conn, conn.cursor() as c:
        ...
        CREATE TABLE IF NOT EXISTS trivia_questions (
            id SERIAL PRIMARY KEY,
            category TEXT NOT NULL CHECK (category IN ('Arte','Ciencia','Geografía','Historia','Deporte','Entretenimiento')),
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_trivia_q_cat_diff ON trivia_questions (category, difficulty)")

        c.execute("""
        CREATE TABLE IF NOT EXISTS trivia_recent_used (
        ...
        c.execute("""
        CREATE TABLE IF NOT EXISTS trivia_packs_daily (
            date_local TEXT PRIMARY KEY,            -- 'YYYY-MM-DD' (Madrid)
            seed TEXT NOT NULL,
            question_ids JSONB NOT NULL,            -- ids barajados del día (cat/diff mezcladas)
        ...
        CREATE TABLE IF NOT EXISTS trivia_games (
            id TEXT PRIMARY KEY,                    -- ej. 'duo_2025-10-21'
            mode TEXT NOT NULL CHECK (mode IN ('duo','duel','blitz','boss')),
            players JSONB NOT NULL,                 -- ["mochito","mochita"]
            status TEXT NOT NULL CHECK (status IN ('active','won','lost','abandoned')) DEFAULT 'active',
            turn_of TEXT,                           -- 'mochito' | 'mochita' (duo/duel)
            crowns JSONB,                           -- {"Arte":0,...} (duo/duel)
            lives INTEGER,                          -- 3 en Dúo, 2 en Boss
            scoreboard JSONB,                       -- {"mochito":0,"mochita":0}
            roulette_history JSONB,                 -- ["Arte","⭐",...]
            duo_day TEXT,                           -- 'YYYY-MM-DD' para el Dúo de ese día
            pack_date TEXT,                         -- 'YYYY-MM-DD' del pack activo
            sync_window_hours INTEGER DEFAULT 6,
            created_at TEXT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS trivia_answers (
            id SERIAL PRIMARY KEY,
            game_id TEXT NOT NULL REFERENCES trivia_games(id) ON DELETE CASCADE,
            question_id INTEGER NOT NULL REFERENCES trivia_questions(id) ON DELETE CASCADE,
            user_name TEXT NOT NULL,
            selected_index INTEGER,
            correct BOOLEAN,
            response_ms INTEGER,
            server_recv_ts TEXT NOT NULL,
            client_sent_ts TEXT,
            ip_hash TEXT,
            order_seed TEXT
        )
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_trivia_answers_game ON trivia_answers (game_id)")
        ...
        CREATE TABLE IF NOT EXISTS trivia_rewards (
            ...
            type TEXT NOT NULL,                     -- coins|xp|cosmetic|wildcard|badge|multiplier
            value TEXT,
            amount INTEGER,
            source TEXT NOT NULL,                   -- chest|boss|mission|bingo|game
            created_at TEXT NOT NULL
        )
        """)

        c.execute("""
        CREATE TABLE IF NOT EXISTS trivia_leaderboard_week (
            week_iso TEXT NOT NULL,                 -- 2025-W43
            user_name TEXT NOT NULL,
            mode TEXT NOT NULL CHECK (mode IN ('blitz')),
            best_score INTEGER NOT NULL,
            updated_at TEXT NOT NULL,
            PRIMARY KEY (week_iso, user_name, mode)
        )
        """)

        # índices de apoyo
        c.execute("CREATE INDEX IF NOT EXISTS idx_trivia_games_status ON trivia_games (status)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_trivia_pack_date ON trivia_packs_daily (date_local)")
...
# ========== [TRIVIA] Helpers de negocio ==========

def trivia__ensure_pack_for(day: date) -> dict | None:
    """Garantiza que existe un pack diario para 'day'. Si no hay preguntas suficientes, devuelve None."""
    dtxt = day.isoformat()
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT date_local, seed, question_ids, expires_at FROM trivia_packs_daily WHERE date_local=%s", (dtxt,))
            row = c.fetchone()
            if row:
                return dict(row)

            # preparar selección básica: coger un buen bloque barajado
            c.execute("""
            ...
            if len(ids) < 60:   # mínimo razonable para el día (lo puedes ajustar)
                return None

            seed = hashlib.sha256(f"{dtxt}::mochitos".encode()).hexdigest()
            ques_ids = _shuffle_with_seed(ids, seed)
            expires = (datetime.combine(day, datetime.min.time()) + timedelta(days=1)).strftime("%Y-%m-%d 10:00:00")

            c.execute("""
              INSERT INTO trivia_packs_daily (date_local, seed, question_ids, expires_at)
              VALUES (%s,%s,%s,%s)
            """, (dtxt, seed, json.dumps(ques_ids), expires))
            conn.commit()
            return {"date_local": dtxt, "seed": seed, "question_ids": ques_ids, "expires_at": expires}
    finally:
        conn.close()

def trivia__duo_game_id_for(day: date) -> str:
    return f"duo_{day.isoformat()}"

def trivia__ensure_duo_game_for(day: date) -> dict | None:
    """Crea/recupera el Dúo Diario del día (3 vidas, 0 coronas)."""
    gid = trivia__duo_game_id_for(day)
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT * FROM trivia_games WHERE id=%s", (gid,))
            row = c.fetchone()
            if row:
                return dict(row)

            pack = trivia__ensure_pack_for(day)
            if not pack:
                return None

            crowns = {k: 0 for k in TRIVIA_CATEGORIES}
            scoreboard = {"mochito": 0, "mochita": 0}
            players = ["mochito","mochita"]

            c.execute("""
              INSERT INTO trivia_games
                (id, mode, players, status, turn_of, crowns, lives, scoreboard, roulette_history,
                 duo_day, pack_date, sync_window_hours, created_at, updated_at)
              VALUES
                (%s,'duo',%s,'active','mochito',%s,3,%s,%s,%s,%s,%s,%s,%s)
              RETURNING *
            """, (gid, json.dumps(players), json.dumps(crowns), json.dumps(scoreboard),
                  json.dumps([]), day.isoformat(), day.isoformat(), TRIVIA_SYNC_WINDOW_HOURS,
                  now_madrid_str(), now_madrid_str()))
            conn.commit()
            row = c.fetchone()
            return dict(row)
    finally:
...
def trivia__pick_category_for(game: dict) -> str:
    """Ruleta simple: 1/7 '⭐' (corona directa), resto categoría aleatoria equiprobable."""
    hist = (game.get("roulette_history") or []) if isinstance(game, dict) else []
    seed = f"{game['id']}::{len(hist)}"
    star = (_rand_from_seed(seed, 0, 6) == 0)
    if star:
        return "⭐"
    idx = _rand_from_seed(seed+"cat", 0, len(TRIVIA_CATEGORIES)-1)
    return TRIVIA_CATEGORIES[idx]

def trivia__get_question(candidate_ids: list[int], user: str) -> dict | None:
    """Devuelve una pregunta no repetida (últimos 30 días) para el usuario."""
    day_limit = (today_madrid() - timedelta(days=30)).isoformat()
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("""SELECT question_id FROM trivia_recent_used
                         WHERE user_name=%s AND used_date >= %s""", (user, day_limit))
            used = {r[0] for r in c.fetchall()}

            # recorrer ids hasta encontrar una activa no usada reciente
            for qid in candidate_ids:
                if qid in used:
                    continue
                c.execute("""SELECT id, category, difficulty, text, choices, correct_index
                             FROM trivia_questions WHERE id=%s AND active=TRUE""", (qid,))
                row = c.fetchone()
                if row:
                    return dict(row)
            return None
...
def trivia__score_for(answer_ok: bool, remaining_ms: int, racha_ind: int, racha_conj: int, sync_ok: bool) -> int:
    base = 100 if answer_ok else 0
    bonus = max(0, int((remaining_ms/1000.0) * 6)) if answer_ok else 0
    penalty = 0 if answer_ok else 20  # 20 por fallo (timeout lo trataremos como fallo duro en validación)
    m_racha = 1 + min(0.50, 0.02*max(0,racha_ind) + 0.01*max(0,racha_conj))
    m_sync = 1.10 if sync_ok else 1.0
    pts = int(round((base + bonus) * m_racha * m_sync)) - penalty
    return max(0, pts)
...
def run_scheduled_jobs(now=None):
    ...
    # [TRIVIA] 8) Pack diario y Dúo Diario a las 10:00
    try:
        if is_due_or_overdue(now, TRIVIA_DAILY_RESET_HHMM, 720):
            pack = trivia__ensure_pack_for(today)
            duo = trivia__ensure_duo_game_for(today)
            if pack and duo:
                # push “listo” solo una vez por día
                key = f"trivia_duo_ready::{today.isoformat()}"
                if not state_get(key, ""):
                    send_push_both("🎡 Dúo diario listo", "¡Vuestras coronas os esperan!", url="/#trivia", tag="trivia-duo")
                    state_set(key, "1")
    except Exception as e:
        print("[trivia pack/duo reset]", e)

    # [TRIVIA] 9) Lunes 10:00: reset ranking Blitz semanal
    try:
        if is_due_or_overdue(now, TRIVIA_DAILY_RESET_HHMM, 720) and today.weekday() == 0:
            key = f"trivia_blitz_week_reset::{_today_week_str()}"
            if not state_get(key, ""):
                send_push_both("⚡ Minuto Relámpago", "Ranking semanal reiniciado. ¿Quién manda esta semana?", url="/#trivia-blitz", tag="trivia-blitz")
                state_set(key, "1")
    except Exception as e:
        print("[trivia blitz reset]", e)

    # [TRIVIA] 10) Domingo 10:00: jefe semanal disponible
    try:
        if is_due_or_overdue(now, TRIVIA_DAILY_RESET_HHMM, 720) and today.weekday() == 6:
            key = f"trivia_boss_ready::{today.isoformat()}"
            if not state_get(key, ""):
                send_push_both("👑 Jefe semanal", "6 preguntas, 2 vidas. Premio e impulso del lunes.", url="/#trivia-boss", tag="trivia-boss")
                state_set(key, "1")
    except Exception as e:
        print("[trivia boss announce]", e)
...
@app.post('/trivia/duo/start')
def trivia_duo_start():
    bad = _require_login_json()
    if bad: return bad
    g = trivia__ensure_duo_game_for(today_madrid())
    if not g:
        return jsonify({"ok": False, "error": "no_pack"}), 400
    return jsonify({"ok": True, "game": g})

@app.post('/trivia/roulette/spin')
def trivia_roulette_spin():
    bad = _require_login_json()
    if bad: return bad
    day = today_madrid()
    g = trivia__ensure_duo_game_for(day)
    if not g: return jsonify({"ok": False, "error": "no_game"}), 400

    cat = trivia__pick_category_for(g)
    # actualiza historial
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            hist = (g.get("roulette_history") or [])
            hist.append(cat)
            c.execute("UPDATE trivia_games SET roulette_history=%s, updated_at=%s WHERE id=%s",
                      (json.dumps(hist), now_madrid_str(), g['id']))
            conn.commit()
    finally:
        conn.close()

    try:
        broadcast("trivia_state", {"game_id": g['id'], "event": "roulette", "value": cat})
    except Exception:
        pass

    return jsonify({"ok": True, "category": cat})

@app.post('/trivia/question/next')
def trivia_question_next():
    bad = _require_login_json()
    if bad: return bad
    user = session['username']
    day = today_madrid()
    ...
    # usamos el pack del día
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT question_ids FROM trivia_packs_daily WHERE date_local=%s", (day.isoformat(),))
            row = c.fetchone()
            if not row:
                return jsonify({"ok": False, "error": "no_pack"}), 400
            ids = row[0] if isinstance(row, dict) else row[0]
    finally:
        conn.close()

    q = trivia__get_question(ids, user)
    if not q:
        return jsonify({"ok": False, "error": "no_question"}), 404

    # orden distinto por jugador
    order_seed = f"{g['id']}::{q['id']}::{user}"
    choices = _shuffle_with_seed(q['choices'], order_seed)
    correct_choice = q['choices'][q['correct_index']]
    correct_index_shuffled = choices.index(correct_choice)
    ...
    payload = {
        ...
        "text": q['text'],
        "choices": choices,
        "time_sec": TRIVIA_TIME_PER_QUESTION_SEC,
        "order_seed": order_seed
    }
    return jsonify({"ok": True, "question": payload, "correct_index_for_server": correct_index_shuffled})
...
def trivia_answer():
    ...
    client_sent = (data.get("client_sent_ts") or "").strip()
    order_seed = (data.get("order_seed") or "").strip()
    issued_at_ms = int(data.get("issued_at_ms") or 0)   # cuando mostramos la pregunta (ms) — lo manda el cliente
    answered_at_ms = int(time.time()*1000)

    if not (game_id and qid and isinstance(selected, int) and 0 <= selected <= 3):
        return jsonify({"ok": False, "error": "bad_payload"}), 400

    # leer juego y correcta
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT * FROM trivia_games WHERE id=%s", (game_id,))
            grow = c.fetchone()
            if not grow:
                return jsonify({"ok": False, "error": "no_game"}), 404
            g = dict(grow)

            c.execute("SELECT id, choices, correct_index FROM trivia_questions WHERE id=%s", (qid,))
            qrow = c.fetchone()
            if not qrow:
                return jsonify({"ok": False, "error": "no_question"}), 404

            base_choices = qrow['choices']
            correct_index = qrow['correct_index']
            # reconstruir el índice correcto con el shuffle del usuario
            shuffled = _shuffle_with_seed(base_choices, order_seed)
            correct_choice = base_choices[correct_index]
            correct_index_user = shuffled.index(correct_choice)

            # validar tiempo (server-side)
            response_ms = max(0, answered_at_ms - issued_at_ms)
            allowed_ms = TRIVIA_TIME_PER_QUESTION_SEC * 1000
            timeout = response_ms > (allowed_ms + 500)  # 500 ms de holgura
            ok = (selected == correct_index_user) and not timeout

            # rachas (simplificado: consulta tus streaks actuales si quieres; aquí asumimos 0)
            racha_ind = 0
            racha_conj = 0
            sync_ok = True  # MVP: puedes calcularlo comparando updated_at de ambos dentro de 6h en la misma partida

            points = trivia__score_for(ok, max(0, allowed_ms - response_ms), racha_ind, racha_conj, sync_ok)

            # actualizar scoreboard, vidas y crowns básicas
            scoreboard = dict(g.get('scoreboard') or {})
            scoreboard[user] = int(scoreboard.get(user, 0)) + points

            lives = int(g.get('lives') or 3)
            if not ok:
                lives = max(0, lives - 1)

            # coronas: si la última ruleta fue "⭐" y acierta -> subimos 1 corona en categoría elegida por cliente (si la envía)
            crowns = dict(g.get('crowns') or {k:0 for k in TRIVIA_CATEGORIES})
            last_cat = None
            try:
                hist = list(g.get('roulette_history') or [])
                last_cat = hist[-1] if hist else None
            except Exception:
                last_cat = None

            chosen_for_star = (data.get("crown_category") or "").strip()
            if last_cat == "⭐" and ok:
                target_cat = chosen_for_star if chosen_for_star in TRIVIA_CATEGORIES else None
                if target_cat:
                    crowns[target_cat] = min(1, int(crowns.get(target_cat, 0)) + 1)  # MVP: 0/1 por categoría
            # victoria si 6 coronas (una por categoría)
            status = 'won' if all(int(crowns.get(k,0)) >= 1 for k in TRIVIA_CATEGORIES) else ('lost' if lives==0 else 'active')

            # persistir todo + event
            c.execute("""
              UPDATE trivia_games
                 SET scoreboard=%s, lives=%s, crowns=%s, status=%s, updated_at=%s
               WHERE id=%s
            """, (json.dumps(scoreboard), lives, json.dumps(crowns), status, now_madrid_str(), game_id))

            c.execute("""
              INSERT INTO trivia_answers (game_id, question_id, user_name, selected_index, correct, response_ms,
                                          server_recv_ts, client_sent_ts, ip_hash, order_seed)
              VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """, (game_id, qid, user, selected, ok, response_ms, now_madrid_str(), client_sent, _hash_ip(ip), order_seed))

            # marcar question usada por user (no repetir 30 días)
            c.execute("""
              INSERT INTO trivia_recent_used (user_name, question_id, used_date)
              VALUES (%s,%s,%s)
              ON CONFLICT (user_name, question_id) DO UPDATE SET used_date=EXCLUDED.used_date
            """, (user, qid, today_madrid().isoformat()))
            conn.commit()

        # avisos y SSE
        try:
            broadcast("trivia_answer", {"game_id": game_id, "user": user, "ok": bool(ok), "points": int(points), "status": status})
        except Exception:
            pass
        if status == 'won':
            try:
                send_push_both("👏 ¡Dúo completado!", "Cofre diario disponible", url="/#trivia", tag="trivia-duo-win")
            except Exception:
                pass

        return jsonify({"ok": True, "correct": bool(ok), "points": int(points),
                        "lives": lives, "crowns": crowns, "status": status})
    except Exception as e:
        print("[trivia_answer]", e)
        try:
            conn.rollback()
        except Exception:
            pass
...
@app.post('/trivia/chest/claim')
def trivia_chest_claim():
    bad = _require_login_json()
    if bad: return bad
    user = session['username']
    day = today_madrid().isoformat()
    # bandera por día y usuario
    key = f"trivia_chest_claimed::{day}::{user}"
    if state_get(key, ""):
        return jsonify({"ok": False, "error": "already_claimed"}), 400

    # RNG cofre: 50% comodín, 30% cosmético, 20% monedas
    r = _rand_from_seed(f"{user}::{day}::chest", 1, 100)
    reward = None
    if r <= 50:
        kind = random.choice(["+10s","cambiar_categoria","pásala_a_tu_pareja"])
        reward = {"type":"wildcard","value":kind}
    elif r <= 80:
        kind = random.choice(["sticker_corazon","marco_oro","banner_estrellas"])
        reward = {"type":"cosmetic","value":kind}
    else:
        amt = _rand_from_seed(f"{user}::{day}::coins", 20, 60)
        reward = {"type":"coins","amount":amt}
    ...
    try:
        with conn.cursor() as c:
            if reward["type"] == "coins":
                c.execute("""INSERT INTO trivia_rewards (user_name, pair, type, amount, source, created_at)
                             VALUES (%s,FALSE,'coins',%s,'chest',%s)""",
                          (user, reward["amount"], now_madrid_str()))
            else:
                c.execute("""INSERT INTO trivia_rewards (user_name, pair, type, value, source, created_at)
                             VALUES (%s,FALSE,%s,%s,'chest',%s)""",
                          (user, reward["type"], reward.get("value"), now_madrid_str()))
            conn.commit()
    finally:
        conn.close()
    ...
    state_set(key, "1")
    try:
        broadcast("trivia_reward", {"user": user, "reward": reward})
    except Exception:
        pass
    return jsonify({"ok": True, "reward": reward})

# ----- Minuto Relámpago (Blitz) -----

@app.post('/trivia/blitz/start')
def trivia_blitz_start():
    bad = _require_login_json()
    if bad: return bad
    user = session['username']
    week = _today_week_str()
    gid = f"blitz_{user}_{int(time.time())}"
    nowtxt = now_madrid_str()
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            players = [user]
            c.execute("""
              INSERT INTO trivia_games
                (id, mode, players, status, turn_of, crowns, lives, scoreboard, roulette_history,
                 duo_day, pack_date, sync_window_hours, created_at, updated_at)
              VALUES
                (%s,'blitz',%s,'active',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """, (gid, json.dumps(players), user, json.dumps({}), None, json.dumps({user:0}),
                  json.dumps([]), None, today_madrid().isoformat(), TRIVIA_SYNC_WINDOW_HOURS, nowtxt, nowtxt))
            conn.commit()
        return jsonify({"ok": True, "game_id": gid, "time_sec": 60})
    finally:
        conn.close()

@app.post('/trivia/blitz/next')
def trivia_blitz_next():
    bad = _require_login_json()
    if bad: return bad
    user = session['username']
    data = request.get_json(silent=True) or {}
    game_id = (data.get("game_id") or "").strip()
    if not game_id: return jsonify({"ok": False, "error": "bad_game"}), 400

    # servir pregunta cualquiera del pack de hoy
    day = today_madrid()
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("SELECT question_ids FROM trivia_packs_daily WHERE date_local=%s", (day.isoformat(),))
            row = c.fetchone()
            if not row: return jsonify({"ok": False, "error": "no_pack"}), 400
            ids = row[0] if isinstance(row, dict) else row[0]
    finally:
        conn.close()
    ...
    order_seed = f"{game_id}::{q['id']}::{user}"
    choices = _shuffle_with_seed(q['choices'], order_seed)
    correct_choice = q['choices'][q['correct_index']]
    correct_index_shuffled = choices.index(correct_choice)
    ...
    return jsonify({"ok": True, "question": {
        ...
        "time_sec": TRIVIA_TIME_PER_QUESTION_SEC,
        "order_seed": order_seed
    }, "correct_index_for_server": correct_index_shuffled})

@app.post('/trivia/blitz/finish')
def trivia_blitz_finish():
    bad = _require_login_json()
    if bad: return bad
    user = session['username']
    data = request.get_json(silent=True) or {}
    ...
    # guardar mejor marca semanal
    conn = get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute("""INSERT INTO trivia_leaderboard_week (week_iso, user_name, mode, best_score, updated_at)
                         VALUES (%s,%s,'blitz',%s,%s)
                         ON CONFLICT (week_iso, user_name, mode) DO UPDATE
                         SET best_score = GREATEST(trivia_leaderboard_week.best_score, EXCLUDED.best_score),
                             updated_at = EXCLUDED.updated_at
            """, (week, user, score, nowtxt))
            conn.commit()
        return jsonify({"ok": True})
    finally:
        conn.close()
