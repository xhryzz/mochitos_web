...
# === Rendimiento inline (<512MB-friendly) ===
import os, re, io, gzip, hashlib
...
_HTML_RE = re.compile(rb"^\s*<!doctype html", re.I)

def _is_text_type(ct: str) -> bool:
    if not ct:
        return False
    ct = ct.lower().split(";")[0].strip()
    return ct.startswith("text/") or ct in {"application/json","application/javascript","application/xml","image/svg+xml","application/manifest+json"}

def _minify_html(html: bytes) -> bytes:
    try:
        s = html.decode("utf-8", "ignore")
        s = re.sub(r'<img(?![^>]*\bloading=)', r'<img loading="lazy" decoding="async"', s, flags=re.I)
        s = re.sub(r'<script(?![^>]*(defer|async)|[^>]*type="module")', r'<script defer', s, flags=re.I)
...
class _OptimizingMiddleware:
    def __init__(self, app, *, gzip_min_len=600, gzip_level=6, max_transform_len=1_500_000):
        self.app = app
        self.gzip_min_len = gzip_min_len
        self.gzip_level = gzip_level
        self.max_len = max_transform_len
    ...
    def __call__(self, environ, start_response):
        ...
        def sr(status, response_headers, exc_info=None):
            ...
            headers_holder['headers'] = list(response_headers)
            headers_holder['exc_info'] = exc_info
        ...
        status = headers_holder.get('status', '200 OK')
        headers = headers_holder.get('headers', [])
        headers_dict = {k.lower(): v for k, v in headers}
        body = b"".join(body_chunks)

        # No tocar SSE/streams
        ...
        # Strip Set-Cookie en estáticos
        if path.startswith("/static/"):
            headers = [(k, v) for (k, v) in headers if k.lower() != "set-cookie"]

        # Cache policies
        if path.startswith("/static/"):
            _cc = "public, max-age=31536000, immutable"
            headers = [(k, v) for (k, v) in headers if k.lower() != "cache-control"]
            headers.append(("Cache-Control", _cc))
        elif path in ("/sw.js", "/manifest.webmanifest") or path.endswith("/sw.js"):
            _cc = "no-cache"
            headers = [(k, v) for (k, v) in headers if k.lower() != "cache-control"]
            headers.append(("Cache-Control", _cc))

        # ETag & 304
        if method in ("GET","HEAD") and len(body) <= self.max_len:
        ...
        # HTML min + reescritura a .min.* si existen físicamente
        if _is_text_type(ctype) and _HTML_RE.match(body[:120]):
            body = _minify_html(body)
            try:
                def maybe_to_min(url: str) -> str:
                    if "/static/" not in url: return url
                    base, ext = os.path.splitext(url)
                    if ext not in (".css", ".js"): return url
                    min_url = base + ".min" + ext
                    # map URL to filesystem
                    if min_url.startswith("/"):
                        fs_path = os.path.join(self._root, min_url.lstrip("/"))
                    else:
                        fs_path = os.path.join(self._root, min_url)
                    return min_url if os.path.exists(fs_path) else url

                s = body.decode("utf-8", "ignore")
                s = re.sub(r'href="([^"]+)"', lambda m: f'href="{maybe_to_min(m.group(1))}"', s)
                s = re.sub(r'src="([^"]+)"', lambda m: f'src="{maybe_to_min(m.group(1))}"', s)
                body = s.encode("utf-8")
            except Exception:
                pass

        # GZIP
        if "gzip" in (accept_enc or "").lower() and _is_text_type(ctype) and len(body) >= self.gzip_min_len:
            out = io.BytesIO()
            with gzip.GzipFile(fileobj=out, mode="wb", compresslevel=self.gzip_level) as gz:
                gz.write(body)
            gz_body = out.getvalue()
        ...
        def set_header(k, v):
            nonlocal headers
            low = k.lower()
            headers = [(hk, hv) for (hk, hv) in headers if hk.lower() == low and False] + [(k, v)]
...
# Activar middleware y ajustes prudentes
try:
    app.wsgi_app = _OptimizingMiddleware(app.wsgi_app)
    ...
    try: print("[perf inline] no-op:", _e)
    except Exception: pass
